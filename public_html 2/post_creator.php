...
$(document).ready(function(){
    ...
    $('.load-more').click(function(){
        ...
        if(row <= allcount){
            ...
            $.ajax({
                url: '<?=BASEPATH?>/include/user_CreatorProfileLoadMore.php',
                type: 'post',
                data: {row:row},
                beforeSend:function(){
...
$('.hide_div').hide();
	 ...
	 $('.load-more1').click(function(){
        var row = Number($('#row1').val());
        var allcount = Number($('#all1').val());
        row = row + <?=PAGINATION?>;
        ...
        if(row <= allcount){
            ...
            $.ajax({
                url: '<?=BASEPATH?>/include/user_CreatorProfile_pactLoadMore.php',
                type: 'post',
                data: {row1:row},
                beforeSend:function(){
...
$('.hide_div1').hide();
                            // Change the text and background
                            $('.load-more1').text("No More Result");
            ...
            setTimeout(function() {
                ...
                // When row is greater than allcount then remove all class='post' element after 3 element
                $('.post1:nth-child(<?=PAGINATION?>)').nextAll('.post1').remove().fadeIn("slow");
