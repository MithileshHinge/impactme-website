<?php $sql_post = $db_query->runQuery("select * from impact_post where user_id='$row_user1->user_id'  order by create_date desc");
            ...
            <div > <iframe frameborder="0"  width="100%" height="351" src="<?= $db_query->getYoutubeEmbedUrl($row_post['video_link'])?>"></iframe> 
             ...
             <?php }?>
                ...
                <p class="like" style="margin-left:20px;font-weight:bold;"><?=$row_post['create_date']?><span class="user-like"></span> <span class="post-like"><a href="javascript:void(0)" id="post_like" onclick="javascript:post_like(<?=$row_post['post_id']?>,<?=$row_user->user_id?>)"> <i class="fa fa-heart"></i> </a><span id="postLikeText<?=$row_post['post_id']?>"> <?=$sql_like_count->c?></span> Likes </span></p>
