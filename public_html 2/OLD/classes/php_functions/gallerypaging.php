...
$max_noof_sets = ceil(($totalrecords / RECORD_PER_PAGE_GALLERY )/PAGE_LINKS_PER_PAGE);
$cur_page_recs = ($totalrecords - ((PAGE_LINKS_PER_PAGE*RECORD_PER_PAGE_GALLERY)*($set-1)));
...
if($page <= 1){
	...
	$page_link .= "<li><a href=\"".HTACCESS_URL."$var_extra/$prev_set/$prevpage/\" ><i class=\"fa fa-chevron-left\"></i></a></li>";
}
...
for($i=1;$i<=PAGE_LINKS_PER_PAGE;$i++){
	...
	$page_link .= "<li class=\"".$stylecolor."\"><a href=\"".HTACCESS_URL."$var_extra/$set/$start_page/\" >".$start_page."</a></li>";

	if(($i*RECORD_PER_PAGE_GALLERY)>=$cur_page_recs) break;
	$start_page++;
}
...
if($nextpage > $start_page && $set==$max_noof_sets){
	$page_link .= "";
 	...
 	if($nextpage != $page)
 		$page_link .= "<li><a href=\"".HTACCESS_URL."$var_extra/$next_set/$nextpage/\" ><i class=\"fa fa-chevron-right\"></i></a></li>";
}

//the output variables
$start_limit = (($page-1)* RECORD_PER_PAGE_GALLERY);
$page_limit = " LIMIT $start_limit,". RECORD_PER_PAGE_GALLERY;

$num_limit = ($page-1) * RECORD_PER_PAGE_GALLERY;
$pagerec = $num_limit;
$lastrec = $pagerec + RECORD_PER_PAGE_GALLERY;
$pagerec = $pagerec + 1;
