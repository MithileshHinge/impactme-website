...
if($page <= 1){
	...
	$page_link .= "<a href=\"".SITE_URL."$PHP_SELF?mo=$var_extra&set=$prev_set&page=$prevpage\" class=\"page dark gradient\">Prev</a>";
}
...
for($i=1;$i<=PAGE_LINKS_PER_PAGE;$i++){
	...
	$stylecolor = ($start_page != $page)? "page dark gradient" : "page dark active";
	
	$page_link .= "<a href=\"".SITE_URL."$PHP_SELF?mo=$var_extra&set=$set&page=$start_page\" class=\"".$stylecolor."\">".$start_page."</a>";
...
if($nextpage > $start_page && $set==$max_noof_sets){
	$page_link .= "";
 	...
 	if($nextpage != $page)
 		$page_link .= "<a href=\"".SITE_URL."$PHP_SELF?mo=$var_extra&set=$next_set&page=$nextpage\" class=\"page dark gradient\">Next</a>";
}
