...
<div class="wrap-lst-category">
	...
	<?php $sql_tier = $db_query->runQuery("select * from impact_tier where user_id='$row_user1->user_id'  order by tier_price");
	$check_subscription = $db_query->fetch_object("select IFNULL(tier_id, 0) from impact_payment where user_id='".$row_user->user_id."' and creator_id='".$row_user1->user_id."' and (status='authenticated' or status='active')");
