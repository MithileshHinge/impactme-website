...
$total_impact = $db_query->fetch_object("select count(*) c, ifnull(sum(a.b),0) p from (select p.user_id, paid_amount b from impact_payment p where p.creator_id='$row_user->user_id' and (p.status='authenticated' or p.status='active') group by p.subscription_id ) a");
$total_onetime = $db_query->fetch_object("select ifnull(sum(a.b),0) p from (select p.user_id, paid_amount b from impact_pay_onetime p where p.creator_id='$row_user->user_id' and p.status='success' group by p.transaction_id) a");
...
<div class="project-runtime" style="margin: 0 0 18px 0;box-shadow: 0 1px 4px rgba(0,0,0,.1);">
                  ...
                  <hr class="line-image">
              <p class="creater-name" style="text-align:center;    padding: 0 0 0 0; "><span class="creater-125" style="cursor:pointer;" onclick="location.href='<?=$user1_path?>';"><?=stripslashes($row_user->impact_name)?> </span></p>
              ...
              <div >
                <span style="text-align:center;margin:auto;display:block;">
                  <?php if(strlen($row_user->youtube_link)>0) { ?>
                  <a href="<?=$row_user->youtube_link?>" target="_blank"  >
                  ...
                  if(strlen($row_user->facebook_link)>0) { ?>
                  <a href="<?=$row_user->facebook_link?>"  target="_blank" >
                      <span style="">
                 ...
                 if(strlen($row_user->instagram)>0) { ?>
             ...
             <div style="text-align:center;">
                  <p class="creater-count" style="text-align:center; left:0;display:block;margin:0 auto;    margin: 20px 0 0 0;">
                    <span class="creater-125"style="font-size:20px"><?php if (empty($total_impact->c)){
                       echo "0";
                     }else {
                      echo $total_impact->c;
                    } ?></span><br>
                     ...
                     <p class="creater-count" style="text-align:center; left:0;display:block;margin:0 auto    ">
                      <span class="creater-125" style="font-size:20px">₹<?php if (empty($total_impact->p)){
                       echo "0";
                     }else {
                      echo $total_impact->p;
                    } ?></span><br>
