...
$sql_c23 = "select * from impact_post where status=1 and user_id in (select u.user_id id from impact_payment p, impact_user u where u.user_id=p.creator_id and p.user_id in $ids and (p.status='authenticated' or p.status='active') group by p.creator_id) order by create_date desc limit 0,$rowperpage1";
       ...
       $page_path = BASEPATH.'/profile/u/'.$row_userPost->user_id."/";
              ...
              <div class="post-creater-postname">
               ...
               <p class="post-comments"><a href="<?=$tier_link1?>"><?=$row_comment->c?> comments</a></p>
                ...
                <?php } ?>
              <?php } ?>
               <?php if( $allcount1> PAGINATION )  { ?>
                 <div class="box-marked-project project-short short hide_div1">
              <h3  class="load-more1">Load More</h3>
              </div>
             
            <input type="hidden" id="row1" value="0">
            <input type="hidden" id="all1" value="<?php echo $allcount1; ?>">
            <?php }?>
