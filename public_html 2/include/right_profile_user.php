...
$sql_1223=  "select count(*) c, ifnull(sum(a.b),0) p from (select p.user_id, sum(paid_amount) b from impact_payment p where p.creator_id='$row_user1->user_id' and p.paid_status='Success' group by p.user_id) a " ;
...
<div class="project-runtime" style="margin: 0 0 18px 0;" >
                  ...
                  <div class="photo" style="background-image:url(<?=$user_image?>);background-size:cover; "></div> 
                   <hr class="line-image">
              <p class="creater-name" style="text-align:center;    padding: 0 0 0 0; "><span class="creater-125"><?=stripslashes($row_user1->impact_name)?></span></p>
              ...
              <?php
			   ...
			   if($row_user1->patronage_visibility==1) { ?>
                  <p class="creater-count" style="text-align:center; left:0;display:inline-block;margin:0 auto;    margin: 11px 14px 0 0;"><span class="creater-125"style="font-size:20px"><?=$total_impact->c ?></span><br>
                      ...
                      <p class="creater-count" style="text-align:center; left:0;display:inline-block;margin:0 auto    "><span class="creater-125" style="font-size:20px"><?=CURRENCY?> <?=$total_impact->p ?></span><br>
                        Per month</p>
                        <?php } ?> 
