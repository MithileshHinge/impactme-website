...
//alert(post_id);
   ...
   var str = $("#frm-comment"+post_id).serialize();
    ...
    str_chk = str.replace(/\+/g, '%20');
    if(decodeURIComponent(str_chk.split("&", 2)[0].split("=", 2)[1]).replace(/^\s+|\s+$/g, '') != ""){
            ...
            function (data) {
                ...
                for (var i = 0; (i < data.length); i++)
                {
                    ...
                    if (parent == "0")
                    {
						...
						"<div class='media-body'><h4 class='rs comment-author'><a href='#' class='fw-b'>"+data[i]['comment_sender_name']+"</a><span class='fc_comment_time'> "+data[i]['date']+" </span></h4><p class='rs comment-content'>"+data[i]['comment']+"</p><p class='rs reply-post'><a class='btn-reply fc-gray be-fc-orange'  onClick='postReply(" + commentId + " , "+ post_id +", \""+fname+"\", "+data[i]['user_id']+" )' title='Reply'><i class='fa fa-reply'></i> Reply</a>"+ comments1 + "<a class='btn-reply fc-gray be-fc-orange'  onClick='postLike(" + commentId + " , "+ post_id +")' title='Like/Dislike'>"+icon+" "+no_of_like+"</a> </p></div>"+
